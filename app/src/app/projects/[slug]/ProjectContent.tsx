...
export default function ProjectContent({ project }: { project: Project }) {
    ...
    return (
        <main className="relative min-h-screen overflow-hidden">
            ...
            <section className="relative pt-32 pb-20 md:pt-40 md:pb-32 flex flex-col justify-center overflow-hidden">
                <motion.div
                    style={{ y, opacity }}
                ...
                <div className="relative z-10 max-w-6xl mx-auto px-6 w-full">
                    ...
                    >
                            ...
                            {project.links ? (
                                project.links.map((link, idx) => (
                                    <a
                                        ...
                                        className={`flex items-center gap-2 px-6 py-3 rounded-lg font-semibold transition-all hover:scale-[1.03] ${link.url.includes('play.google.com') ? 'bg-primary text-white hover:bg-primary-soft shadow-lg shadow-primary/20' : 'bg-accent text-white hover:bg-accent/80 shadow-lg shadow-accent/20'}`}
                                    >
                                ...
                                <a
                                    ...
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    className={`flex items-center gap-2 px-6 py-3 rounded-lg font-semibold transition-all hover:scale-[1.03] ${project.link.includes('play.google.com') ? 'bg-primary text-white hover:bg-primary-soft shadow-lg shadow-primary/20' : 'bg-white text-background hover:bg-white/90 shadow-lg shadow-white/20'}`}
                                >
                        ...
                        {/* Tech Stack Pills alongside buttons */}
                        <div className="flex flex-wrap gap-2 items-center ml-auto">
                            {project.techStack.map((tech) => (
                                <span
                                    key={tech.name}
                                    className="px-3 py-1 rounded-full bg-white/[0.04] border border-white/8 text-sm font-medium text-foreground-muted/80 whitespace-nowrap"
                                >
                                    {tech.name}
                                </span>
                            ))}
            ...
            {project.details.metrics && project.details.metrics.length > 0 && (
                <section className="relative z-20 -mt-8 mb-16 max-w-6xl mx-auto px-6">
                    <div className="glass-panel rounded-2xl p-8 md:p-10 flex flex-wrap gap-8 md:gap-16 justify-center md:justify-around text-center border-l-4 border-l-primary">
                        {project.details.metrics.map((metric, idx) => (
                            <div key={idx} className="flex flex-col">
                                <span className="text-3xl md:text-5xl font-bold text-foreground tracking-tight mb-2">
                                    {metric.value}
                                </span>
            ...
            {/* Narrative Block: The Challenge, The Approach, The Impact */}
            <Section className="py-20">
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-16">

                    {/* Left Column: Context / Full Description */}
                ...
                <div className="relative z-10">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {project.details.features.map((feature, idx) => (
                            ...
                            >
                                <span className="absolute top-6 right-6 text-5xl font-bold text-white/[0.02] group-hover:text-primary/[0.05] transition-colors select-none">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
                        {project.details.timeline.map((item, idx) => (
                            <motion.div
                                ...
                                className="relative"
                            >
                                <div className="hidden md:block absolute top-1.5 left-0 w-full h-[2px] bg-white/10 -z-10" />
                                <div className="w-3 h-3 rounded-full bg-primary mb-6 shadow-[0_0_10px_rgba(249,115,22,0.5)] border-2 border-background" />
                                <span className="text-primary font-mono text-xs uppercase tracking-wider font-semibold mb-2 block">{item.date}</span>
